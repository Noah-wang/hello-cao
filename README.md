...
对于比赛表现、当天赛果等时效问题，Bot 会为查询补充日期和赛后关键词，并检查搜索资料是否与问题、日期和比赛阶段匹配。若首轮只找到赛前前瞻或跑题资料，会自动修正关键词重搜一次；预测类问题即使资料有限，也会明确给出带不确定性说明的主观预测。

在消息中附带 B站链接、`b23.tv` 短链接或 BV 号时，Bot 会读取视频标题、简介和中文/AI 字幕。只发送链接会自动总结，链接后附带问题则基于字幕回答；没有可用字幕时只根据标题和简介回答并明确说明限制。B站凭据通过 `BILIBILI_CREDENTIAL_PATH` 指向服务器上的私密 TOML 文件，不提交到 Git。
...
用户明确说“搜索、搜一下、查一下”，或问题涉及最新、今天、新闻、价格、天气、比分等实时信息时，Bot 会直接调用 Monid。关键词无法确定时，Bot 会先让 LLM 做一次最多 8 秒的语义判断，因此也能识别信用卡积分、航空里程、航线、冷门事实等没有固定关键词但需要最新资料的问题。判断失败时退回关键词规则，不会阻塞正式回答。搜索最多获取 3 个网页来源；网页内容按不可信资料处理，不能覆盖 system prompt；回答会使用 `[1]`、`[2]` 标注来源。
